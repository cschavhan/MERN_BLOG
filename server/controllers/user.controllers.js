...
export const update = async (req, res, next) => {
  ...
  if (req.body.password) {
    if (req.body.password.length < 6) {
      return next(errorHandler(400, "Password must be atleast 6 charactres"));
    }
  ...
  if (req.body.username) {
    ...
    if (req.body.username.includes(" ")) {
      return next(errorHandler(400, "Username cannot cantain spaces"));
    }
...
export const getUsers = async (req, res, next) => {
  if (!req.user.isAdmin) {
    next(errorHandler(403, "You are not allowed to see all the users"));
  }
